...
/**
 * @brief Represents a multi-dimensional array (tensor) of floating-point numbers.
 *
...
public:
    ...
    /**
     * @brief The underlying data of the tensor, stored as a contiguous vector of floats.
     *
     * The elements are typically ordered according to a row-major (C-style) layout,
     * though this class itself doesn't enforce a specific layout.
     */
     ...
     * total number of elements in the tensor (product of the shape dimensions),
     * but the values will be uninitialized.
     *
    ...
    /**
     * @brief Constructs a Tensor with the specified shape and initializes it with the provided data.
     *
     * The size of the data vector must exactly match the total number of elements
     ...
     *
     * Currently, this destructor doesn't perform any specific memory management
     * as the `std::vector` handles the deallocation of the underlying data.
     * However, it is good practice to include a destructor in case future
     * versions of the class require custom cleanup.
     *
     ...
     *
     * @return A py::array_t<float> object containing a copy of the tensor's data
     * with the same shape.
     */
