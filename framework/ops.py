...
def matmul(x, y):
    ...
    Example:
        ...
        >>> x = tensor.Tensor.from_numpy(x_np)
        >>> y = tensor.Tensor.from_numpy(y_np)
        >>> result = matmul(x, y)
        >>> print(result.numpy())
        [[19. 22.]
         [43. 50.]]
...
def add_(x, y):
    ...
    Args:
        x: `framework.tensor.Tensor`. A tensor of shape (m, n).
        y: `framework.tensor.Tensor`. A tensor of shape (m, n).
...
def add(x, y):
    ...
    Args:
        x: `framework.tensor.Tensor`. A tensor of shape (m, n).
        y: `framework.tensor.Tensor`. A tensor of shape (1, n).

    Returns:
        A `framework.tensor.Tensor` with shape (m, n). The result of the
...
def multiply(x, y):
    ...
    Args:
        x: `framework.tensor.Tensor`. A tensor of shape (m, n).
        y: `framework.tensor.Tensor`. A tensor or shape (m, n) or a single
            float.
...
def relu(x):
    ...
    Args:
        x: `framework.tensor.Tensor`.

    Returns:
        A `framework.tensor.Tensor`. The result of the ReLU operation.
...
def softmax(x):
    ...
    Args:
        x: `framework.tensor.Tensor`. The input tensor of shape (m, n), where m
...
def log(x):
    ...
    Returns:
        A `framework.tensor.Tensor`. The result of the log operation.
...
def sum(x):
    ...
    Args:
        x: `framework.tensor.Tensor`. The input tensor of shape (m, n).

    Returns:
        A `framework.tensor.Tensor` with shape (1,). The sum of all elements in
        x.
